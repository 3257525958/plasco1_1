...
#
from account_app.models import InventoryCount, Branch, ProductPricing
from .models import Invoicefrosh, InvoiceItemfrosh, POSDevice, CheckPayment, CreditPayment, CashPayment, POSTransaction,ReturnLog
from .forms import BranchSelectionForm, POSDeviceForm, CheckPaymentForm, CreditPaymentForm
#
...
#     branches = Branch.objects.all()
#     today = datetime.now().date()  # تغییر این خط
...
#                     gregorian_date = jalali_date.togregorian()
#
#                     # اصلاح این بخش: استفاده از dt به جای datetime_module
#                     start_date = timezone.make_aware(
#                         datetime.combine(gregorian_date, datetime.min.time()))
#                     end_date = timezone.make_aware(datetime.combine(gregorian_date, datetime.max.time()))
#
...
#     # تاریخ امروز به شمسی
#     today = datetime.now().date()  # یا today = dt.now().date()
#     today_jalali = jdatetime.fromgregorian(date=today).strftime('%Y/%m/%d')
#     # دریافت پارامترهای فیلتر
#     branch_id = request.GET.get('branch', '')
        ...
        try:
            ...
            try:
                ...
                if len(date_parts) == 3:
                    ...
                    # تبدیل تاریخ شمسی به میلادی - استفاده از jdatetime_datetime
                    jalali_date = jdatetime_datetime(year=year, month=month, day=day)
                    gregorian_date = jalali_date.togregorian().date()

                    # ایجاد رنج تاریخ - استفاده از datetime به جای dt
                    start_date = datetime.combine(gregorian_date, datetime.min.time())
                    end_date = datetime.combine(gregorian_date, datetime.max.time())
        ...
        try:
            ...
            if date_str:
                try:
                    ...
                    # استفاده از jdatetime_datetime
                    jalali_date = jdatetime_datetime(year=year, month=month, day=day)
                    gregorian_date = jalali_date.togregorian().date()

                    # استفاده از datetime به جای dt
                    start_date = timezone.make_aware(
                        datetime.combine(gregorian_date, datetime.min.time()))
                    end_date = timezone.make_aware(datetime.combine(gregorian_date, datetime.max.time()))
...
def daily_invoices(request):
    ...
    if date_filter:
        try:
            ...
            if len(date_parts) == 3:
                ...
                # ایجاد تاریخ شمسی - استفاده از jdatetime_datetime
                jalali_date = jdatetime_datetime(year=year, month=month, day=day)
                ...
                # ایجاد رنج تاریخ - استفاده از datetime به جای dt
                start_of_day = datetime.combine(gregorian_date, datetime.min.time())
                end_of_day = datetime.combine(gregorian_date, datetime.max.time())
    ...
    # دریافت شعبه‌ها
    from cantact_app.models import Branch as CantactBranch
    branches = CantactBranch.objects.all()
