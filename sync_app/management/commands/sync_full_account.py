...
class Command(BaseCommand):
    help = 'انتقال کامل account_app بدون استفاده از پیش‌فرض'
    ...
    def handle(self, *args, **options):
        ...
        # مرحله 1: بررسی و همگام‌سازی وابستگی‌ها اول
        self.stdout.write("\n🔍 مرحله 1: بررسی وابستگی‌ها...")
        dependencies_status = self.check_dependencies()

        if not dependencies_status:
            self.stdout.write("❌ وابستگی‌های لازم وجود ندارند. انتقال متوقف شد.")
            return

        # مرحله 2: دریافت و ذخیره رکوردهای InventoryCount با وابستگی‌ها
        self.stdout.write("\n📦 مرحله 2: انتقال InventoryCount...")
        inventory_result = self.sync_inventory_count_with_validation()

        # مرحله 3: انتقال سایر مدل‌ها
        self.stdout.write("\n📦 مرحله 3: انتقال سایر مدل‌ها...")
        other_results = self.sync_other_models()

        # مرحله 4: گزارش نهایی
        self.stdout.write("\n📊 مرحله 4: گزارش نهایی...")
        self.generate_final_report(inventory_result, other_results)

    def check_dependencies(self):
        """بررسی وجود وابستگی‌های لازم"""
        try:
            from django.contrib.auth.models import User
            from cantact_app.models import Branch
            ...
            user_count = User.objects.count()
            if user_count == 0:
                self.stdout.write("❌ هیچ کاربری در سیستم وجود ندارد.")
                self.stdout.write("✅ راه‌حل: ابتدا دستور sync_full_cantact را اجرا کنید.")
                return False

            # بررسی وجود حداقل یک شعبه
            branch_count = Branch.objects.count()
            if branch_count == 0:
                self.stdout.write("❌ هیچ شعبه‌ای در سیستم وجود ندارد.")
                self.stdout.write("✅ راه‌حل: ابتدا دستور sync_full_cantact را اجرا کنید.")
                return False

            # دریافت IDهای موجود
            self.user_ids = list(User.objects.values_list('id', flat=True))
            self.branch_ids = list(Branch.objects.values_list('id', flat=True))

            self.stdout.write(f"✅ {user_count} کاربر موجود")
            self.stdout.write(f"✅ {branch_count} شعبه موجود")

            return True

        except Exception as e:
            self.stdout.write(f"❌ خطا در بررسی وابستگی‌ها: {e}")
            return False

    def sync_inventory_count_with_validation(self):
        """انتقال InventoryCount با اعتبارسنجی وابستگی‌ها"""
        try:
            ...
            if response.status_code != 200:
                self.stdout.write(f"❌ خطا در دریافت داده‌ها: {response.status_code}")
                return {'transferred': 0, 'skipped': 0, 'errors': 1}

            data = response.json()
            if data.get('status') != 'success':
                self.stdout.write(f"❌ خطا در پاسخ سرور: {data.get('message', 'خطای نامشخص')}")
                return {'transferred': 0, 'skipped': 0, 'errors': 1}

            records = data.get('records', [])
            self.stdout.write(f"📥 دریافت {len(records)} رکورد از سرور")

            # پردازش رکوردها
            return self.process_inventory_records(records)

        except Exception as e:
            self.stdout.write(f"❌ خطا در انتقال InventoryCount: {e}")
            return {'transferred': 0, 'skipped': 0, 'errors': 1}

    def process_inventory_records(self, records):
        """پردازش رکوردهای InventoryCount"""
        from account_app.models import InventoryCount
        ...
        for index, record_data in enumerate(records, 1):
            try:
                # اعتبارسنجی رکورد
                validation_result = self.validate_inventory_record(record_data)

                if not validation_result['valid']:
                    skipped_count += 1
                    self.stdout.write(f"⚠️ رکورد {index}: {validation_result['message']}")
                    continue

                # ذخیره رکورد
                saved = self.save_inventory_record(record_data, validation_result)
                ...
                # نمایش پیشرفت
                if index % 50 == 0:
                    self.stdout.write(f"📝 پردازش {index} از {len(records)} رکورد...")

            except Exception as e:
                error_count += 1
                self.stdout.write(f"❌ خطا در پردازش رکورد {index}: {e}")
    ...
    def validate_inventory_record(self, record_data):
        """اعتبارسنجی رکورد InventoryCount"""
        result = {'valid': False, 'message': '', 'user_id': None, 'branch_id': None}

        try:
            # بررسی فیلدهای اجباری
            required_fields = ['id', 'product_name', 'branch_id', 'counter_id']
            for field in required_fields:
                if field not in record_data or record_data[field] is None:
                    result['message'] = f"فیلد {field} خالی است"
                    return result

            # بررسی وجود branch
            branch_id = record_data['branch_id']
            if branch_id not in self.branch_ids:
                result['message'] = f"شعبه با ID {branch_id} وجود ندارد"
                return result

            # بررسی وجود counter (user)
            counter_id = record_data['counter_id']
            if counter_id not in self.user_ids:
                result['message'] = f"کاربر با ID {counter_id} وجود ندارد"
                return result

            # اعتبارسنجی موفق
            result['valid'] = True
            result['user_id'] = counter_id
            result['branch_id'] = branch_id
            result['message'] = "رکورد معتبر"

        except Exception as e:
            result['message'] = f"خطا در اعتبارسنجی: {e}"

        return result

    def save_inventory_record(self, record_data, validation_result):
        """ذخیره رکورد InventoryCount"""
        try:
            from account_app.models import InventoryCount
            ...
            clean_data = {
                ...
                'profit_percentage': Decimal(str(record_data.get('profit_percentage', 70.00))),
                'branch_id': validation_result['branch_id'],
                'counter_id': validation_result['user_id']
            }
            ...
            self.stdout.write(f"❌ خطا در ذخیره رکورد {record_data.get('id')}: {e}")
            return False

    def sync_other_models(self):
        """انتقال سایر مدل‌های account_app"""
        models_to_sync = [
            'Product', 'ProductPricing', 'PaymentMethod',
        ...
        for model_name in models_to_sync:
            try:
                model_class = apps.get_model('account_app', model_name)
                count = self.sync_single_model_without_deps(model_class)
                results[model_name] = count
                self.stdout.write(f"✅ {model_name}: {count} رکورد انتقال یافت")
            except Exception as e:
                self.stdout.write(f"❌ خطا در انتقال {model_name}: {e}")
    ...
    def sync_single_model_without_deps(self, model_class):
        """انتقال یک مدل بدون وابستگی خارجی"""
        try:
            response = requests.get(
                f"{settings.ONLINE_SERVER_URL}/api/sync/model-data/",
                params={'app': 'account_app', 'model': model_class.__name__},
            ...
            for record_data in records:
                try:
                    ...
                    # حذف فیلدهای وابسته به مدل‌های دیگر
                    record_data.pop('branch_id', None)
                    record_data.pop('counter_id', None)
    ...
    def generate_final_report(self, inventory_result, other_results):
        """تولید گزارش نهایی"""
        self.stdout.write("\n" + "=" * 60)
        self.stdout.write("📋 گزارش نهایی انتقال account_app")
        self.stdout.write("=" * 60)

        # گزارش InventoryCount
        self.stdout.write("\n📊 InventoryCount:")
        self.stdout.write(f"   📤 انتقال یافته: {inventory_result['transferred']} رکورد")
        self.stdout.write(f"   ⏭️  رد شده: {inventory_result['skipped']} رکورد (عدم وجود وابستگی)")
        self.stdout.write(f"   ❌ خطاها: {inventory_result['errors']} رکورد")

        # گزارش سایر مدل‌ها
        total_other = 0
        for model_name, count in other_results.items():
            self.stdout.write(f"\n📊 {model_name}:")
            self.stdout.write(f"   📤 انتقال یافته: {count} رکورد")
            total_other += count

        # جمع کل
        self.stdout.write("\n" + "=" * 60)
        self.stdout.write(f"📈 جمع کل انتقال‌ها: {inventory_result['transferred'] + total_other} رکورد")
        self.stdout.write(f"⏭️  جمع کل رد شده‌ها: {inventory_result['skipped']} رکورد")
        self.stdout.write("=" * 60)

        # توصیه‌ها
        if inventory_result['skipped'] > 0:
            self.stdout.write("\n💡 توصیه‌ها:")
            self.stdout.write("1. برای انتقال رکوردهای رد شده، ابتدا مطمئن شوید که:")
            self.stdout.write("   - تمام کاربران با sync_full_cantact همگام شده‌اند")
            self.stdout.write("   - تمام شعبه‌ها با sync_full_cantact همگام شده‌اند")
            self.stdout.write("2. سپس مجدداً این دستور را اجرا کنید")