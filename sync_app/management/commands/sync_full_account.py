...
#                 continue
#
#             # مدیریت ویژه برای InventoryCount
#             if model_class.__name__ == 'InventoryCount':
#                 if field_name == 'branch_id':
#                     # branch_id رو بررسی کن
...
class Command(BaseCommand):
    help = 'انتقال کامل account_app با دور زدن متد save مشکل‌دار'
    ...
    def sync_inventory_count_safe(self, model_class):
        """انتقال ایمن InventoryCount با دور زدن متد save"""
        try:
            response = requests.get(
                f"{settings.ONLINE_SERVER_URL}/api/sync/model-data/",
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == 'success':
                    records = data.get('records', [])
                    self.stdout.write(f"📥 دریافت {len(records)} رکورد برای InventoryCount")
                    return self.save_inventory_records_raw(model_class, records)

            return 0

        except Exception as e:
            self.stdout.write(f"⚠️ خطا در دریافت داده‌های InventoryCount: {e}")
            return 0

    def save_inventory_records_raw(self, model_class, records):
        """ذخیره مستقیم InventoryCount بدون فراخوانی متد save"""
        from django.db import connection
        from cantact_app.models import Branch
        from django.contrib.auth.models import User
        ...
        # دریافت اولین branch و user برای مقادیر پیش‌فرض
        ...
        for record_data in records:
            try:
                ...
                is_new = record_data.get('is_new', True)
                quantity = record_data.get('quantity', 0)
                count_date = record_data.get('count_date', '1403/01/01')
                barcode_data = record_data.get('barcode_data', '')
                selling_price = record_data.get('selling_price', 0)
                profit_percentage = record_data.get('profit_percentage', 0.0)
                ...
                if not counter_id or not User.objects.filter(id=counter_id).exists():
                    counter_id = default_user.id if default_user else 1

                # استفاده از SQL خام برای درج
                with connection.cursor() as cursor:
                    cursor.execute("""
                    ...
                    """, [
                        record_id, product_name, is_new, quantity, count_date,
                        record_data.get('created_at', '2024-01-01 00:00:00'),
                        barcode_data, selling_price, branch_id, counter_id, profit_percentage
                    ])
                ...
                error_count += 1
                if error_count <= 3:
                    self.stdout.write(f"❌ خطا در رکورد {record_data.get('id')}: {e}")
                continue
