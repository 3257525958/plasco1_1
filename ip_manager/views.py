...
echo Step 5: Syncing data from online server...
echo IMPORTANT: Syncing in correct order to avoid foreign key issues...
python manage.py sync_full_cantact  # First sync branches
timeout /t 3 /nobreak >nul
python manage.py sync_full_account  # Then sync accounts (needs branches)
timeout /t 3 /nobreak >nul
python manage.py sync_full_dashbord
timeout /t 3 /nobreak >nul
python manage.py sync_full_invoice
...
6. Problem: Foreign key constraint fails (InventoryCount)
   Solution:
   - Make sure sync_full_cantact runs before sync_full_account
   - Check that branches and users exist in database
...
- Make migrations: python manage.py makemigrations
- Sync in order:
  1. python manage.py sync_full_cantact
  2. python manage.py sync_full_account
  3. python manage.py sync_full_dashbord
  4. python manage.py sync_full_invoice
  5. python manage.py sync_full_pos_payment
...
- If "hazm" fails, try:
  pip install hazm --no-deps
- For Persian text issues, ensure UTF-8 encoding
- For foreign key issues, run sync commands in correct order
'''
            zipf.writestr('TROUBLESHOOTING.txt', troubleshooting_content)
...
For proper data synchronization, commands run in this order:
1. cantact_app (branches and contacts)
2. account_app (products, inventory, etc.)
3. dashbord_app
4. invoice_app
5. pos_payment
...
- First run may take 5-15 minutes
- Ensure no antivirus is blocking the installation
- For foreign key issues, restart and run sync commands manually
'''
            zipf.writestr('README_FIRST.txt', readme_content)
...
- Package errors: Install packages individually
- Permission errors: Run as Administrator
- Foreign key errors: Run sync commands in this order:
  1. python manage.py sync_full_cantact
  2. python manage.py sync_full_account
  3. python manage.py sync_full_dashbord
  4. python manage.py sync_full_invoice
